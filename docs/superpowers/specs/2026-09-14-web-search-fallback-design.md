...
- **`src/research/webSearch.ts`** (new)
  ...
  - `async function searchWeb(query: string): Promise<WebSearchResult>` — POSTs to Parallel's search API with `Authorization` header from `PARALLEL_API_KEY`, parses the response into `WebSearchResult`. Throws on non-2xx response, network failure, or malformed response body. Pure function: no filesystem or registry access, independently testable via mocked `fetch`.
- **`src/research/evidenceWriter.ts`** (new)
  - `function writeEvidenceMarkdown(gapId: string, query: string, result: WebSearchResult): string` — validates `gapId` against the registry's existing id format (alphanumeric + dash only; reject anything else before it touches a path) and writes a markdown document (query, then each hit as `### {title}` / url / snippet) to `_kb-sync-staging/trm/gap-{gapId}-evidence.md`. Returns the absolute path. Overwrites any existing file at that path (no cross-attempt reuse — see Error handling).
...
3. Else run `nlm research` as today; extract answer text (per the other plan's contract) and check it with `isWeakResearchResult()`. Strong → stop, existing behavior unchanged. Weak → continue.
