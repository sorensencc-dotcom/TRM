...
- **Credential:** `PARALLEL_API_KEY`, read directly via `process.env.PARALLEL_API_KEY` inside `searchWeb` — trm has no dotenv/secrets-loading layer today (confirmed: `config.ts` only reads `config.json`; other trm modules read env vars directly, e.g. `TRM_FFMPEG_CONCURRENCY` in `src/core/concurrency.ts`), so this follows that existing convention rather than adding one. No change to `config.ts`/`TrmConfig`. Value comes from the same key already provisioned at `C:\Users\soren\.secrets\parallel.env` for the `toolforge` project (however the process running trm gets that into its environment — out of scope here, same as any other `TRM_*` env var); no new key needed.
