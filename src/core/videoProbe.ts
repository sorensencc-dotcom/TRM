...
// Custom error class for ffprobe output validation errors (JSON parse, format/stream validation)
// Thrown at validation sites to distinguish from subprocess execution errors
class FfprobeValidationError extends Error {
...
export async function probeVideo(filePath: string): Promise<ProbeResult> {
  ...
  try {
    ...
    try {
      ...
      throw new FfprobeValidationError(`Failed to parse ffprobe JSON output: ${String(parseErr)}`);
    }

    if (!probeData || typeof probeData !== 'object') {
      throw new FfprobeValidationError('Invalid ffprobe output: expected object');
    }
    ...
    if (durationValue === undefined) {
      throw new FfprobeValidationError('Invalid ffprobe output: format.duration is missing');
    }
    ...
    if (typeof durationValue === 'string') {
      ...
      if (isNaN(durationSeconds)) {
        throw new FfprobeValidationError('Invalid ffprobe output: format.duration is not a valid number');
      }
    } else if (typeof durationValue === 'number') {
      durationSeconds = durationValue;
    } else {
      throw new FfprobeValidationError('Invalid ffprobe output: format.duration is not a number or string');
    }
    ...
    if (!Array.isArray(streams)) {
      throw new FfprobeValidationError('Invalid ffprobe output: streams is not an array');
    }
    ...
    // Validation errors (JSON parse, field validation) are already contextualized
    if (err instanceof FfprobeValidationError) {
      throw err;
    }
    // Subprocess execution errors (timeout, non-zero exit, etc.) get wrapped with file context
